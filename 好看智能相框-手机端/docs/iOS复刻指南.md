...
> **目标**：完美复刻项目根目录下 **`index.html`** 的 UI 与交互，供 iOS 原生实现时作为唯一复刻依据。  
> **设计基准**：iPhone 14 Pro，逻辑宽度 390pt，使用 Safe Area 与系统字体。  
> 本文档已汇总 `UI设计规范`、`UI视觉规格文档`、`背景与圆角规格`、`发送任务逻辑说明` 等，复刻时以本文档 + **index.html** + **assets/** 为准即可。
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **可交互原型** | 项目根目录 `index.html` | 所有页面、动效、交互的参考实现，请本地打开逐屏对照 |
| **图标** | `assets/icons/` | SVG，可导入 Asset Catalog 或转为 SF Symbol 使用；详见 `assets/icons/README.md` |
| **背景/圆角** | `assets/drawables/` | 规格与色值同 Android；iOS 用 UIColor + 圆角/阴影代码或 Asset 实现，见 `assets/drawables/README.md`、`docs/背景与圆角规格.md` |
| **欢迎页** | `assets/welcome/` | 欢迎页 Logo、插图、扫码按钮 SVG |

**详细规格**（需查某模块精确数值时）：`docs/UI视觉规格文档.md`、`docs/背景与圆角规格.md`。
...
- **图标**：`assets/icons/` 下 SVG，可按尺寸导入 Asset Catalog（Single Scale 或 @1x），或映射为 SF Symbol 使用；颜色用 tint 或 `currentColor` 语义。
- **欢迎页**：`assets/welcome/` 下三份 SVG，可直接用于 `UIImageView` 或转 PDF 进 Asset。
- **背景/圆角**：无现成 iOS 资源包，按 `assets/drawables/README.md` 与 `docs/背景与圆角规格.md` 用 `UIColor` + 圆角/阴影/渐变代码实现；渐变 135° 可用 `CAGradientLayer`。
...
- **欢迎页**：全屏、Logo/标题/描述/插图/主按钮/跳过，资源来自 `assets/welcome`。
...
| 文档 | 用途 |
|------|------|
| `docs/UI视觉规格文档.md` | 各模块精确数值 |
| `docs/UI设计规范.md` | 完整 Design Token 与组件规范 |
| `docs/背景与圆角规格.md` | 背景/圆角/边框/阴影完整清单 |
| `docs/发送任务逻辑说明文档.md` | 发送任务状态与四层反馈 UI |
| `docs/照片权限交互说明文档.md` | 权限与空态交互 |

**复刻流程建议**：用本文档 + index.html + assets 搭页面与 Token → 按「五、按模块的 UI 规格」与《UI视觉规格文档》对细节 → 按「六、交互要点」实现逻辑 → 用「七、自查清单」收尾核对。
